...
# def worker(q, features, data, s):
#    """
#    This inner function runs the real Feature Extraction task.
#    :param q: the queue in which the data is to be stored.
...
#    :param s: the id of the data chunk passed one parameter before.
#    """
#    for j in range(len(data)):
#        data[j][id] = j + s
#        data_feature = tsfresh.extract_features(data[j], column_id="id", default_fc_parameters=features)
#        q.put(data_feature)
#
#
...
#                                   args=(queue,
#                                         settings,
#                                         X[start:int((i + 1) * chunk_size)].copy(), start))
#    procs.append(proc)
#    proc.start()
#
# results = []
